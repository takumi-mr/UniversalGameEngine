...
import {
  type EnergyType,
  PokemonTCGRegistry,
} from "@engine/shared/rules/PokemonTCG/PokemonTCGRegistry";
// ==========================================
...
export class PokemonTCGRuleset implements GameRuleset<PokemonTCGState, PokemonTCGAction> {
  ...
  public getInitialState(options?: { playerIds?: string[] }, _rng?: IGameRNG): PokemonTCGState {
    ...
    playerIds.forEach((id, idx) => {
      ...
      playerData[id] = {
        id,
        deck: [],
        hand: [isP1 ? "e_lightning" : "e_fire", "t_potion", initialCardId],
        discard: [],
  ...
  public reduce(state: PokemonTCGState, action: PokemonTCGAction, rng?: IGameRNG): PokemonTCGState {
    ...
    if (action.type === "PLAY_BASIC") {
      ...
      if (handIdx > -1) {
        ...
        else if (action.targetZone === "BENCH" && player.bench.length < 5)
  ...
  public checkWinCondition(state: PokemonTCGState): {
    isFinished: boolean;
    winnerIds?: string[];
    message?: string;
  } {
