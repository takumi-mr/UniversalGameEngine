...
export class PokemonPocketRuleset implements GameRuleset<PokemonPocketState, PokemonPocketAction> {
  ...
  public reduce(
    state: PokemonPocketState,
    action: PokemonPocketAction,
    rng?: IGameRNG,
  ): PokemonPocketState {
    ...
    if (action.type === "PLAY_BASIC") {
      ...
      if (handIdx > -1) {
        ...
        else if (action.targetZone === "BENCH" && player.bench.length < 3)
    ...
    if (event.type === "DEAL_DAMAGE") {
      ...
      if (targetPokemon && targetPlayer) {
        ...
        const currentTopCard =
          targetPokemon.evolutionStack[targetPokemon.evolutionStack.length - 1];
      ...
      for (const p of Object.values(state.playerData)) {
        targetPokemon =
          [p.active, ...p.bench].find((poke) => poke?.instanceId === event.targetInstanceId) ||
          undefined;
  ...
  private hasEnoughEnergy(
    attached: EnergyType[],
    cost: Partial<Record<EnergyType, number>>,
  ): boolean {
    const attachedCounts = attached.reduce(
      (acc, type) => {
  ...
  public checkWinCondition(state: PokemonPocketState): {
    isFinished: boolean;
    winnerIds?: string[];
    message?: string;
  } {
