...
export type EnergyType = 'FIRE' | 'LIGHTNING' | 'GRASS' | 'WATER' | 'PSYCHIC' | 'FIGHTING' | 'DARK' | 'METAL' | 'COLORLESS';
...
export interface CardDefinition {
    ...
    attacks?: { name: string; cost: Partial<Record<EnergyType, number>>; damage: number }[];
...
export const PokemonPocketRegistry: Record<string, CardDefinition> = {
    ...
    // 通常のたねポケモン (1ポイント)
    'p_pikachu': {
        id: 'p_pikachu', name: 'Pikachu', category: 'POKEMON', hp: 60, points: 1,
        attacks: [
            { name: 'Thunder Shock', cost: { LIGHTNING: 1 }, damage: 20 },
            { name: 'Electro Ball', cost: { LIGHTNING: 1, COLORLESS: 1 }, damage: 40 }
        ]
    },
    'p_charmander': {
        id: 'p_charmander', name: 'Charmander', category: 'POKEMON', hp: 60, points: 1,
        attacks: [
    ...
    'p_charmeleon': {
        ...
        stage: 'STAGE1', evolvesFrom: 'p_charmander', hp: 90, points: 1,
        attacks: [{ name: 'Flamethrower', cost: { FIRE: 2, COLORLESS: 1 }, damage: 80 }]
    },

    // 2進化 exポケモン (2ポイント)
    'p_charizard_ex': {
        id: 'p_charizard_ex', name: 'Charizard ex', category: 'POKEMON',
        stage: 'STAGE2', evolvesFrom: 'p_charmeleon', hp: 180, points: 2,
        attacks: [{ name: 'Crimson Storm', cost: { FIRE: 2, COLORLESS: 2 }, damage: 200 }]
    },

    // たね exポケモン (2ポイント)
    'p_pikachu_ex': {
        id: 'p_pikachu_ex', name: 'Pikachu ex', category: 'POKEMON', hp: 120, points: 2,
        attacks: [
    ...
    't_pokeball': {
        id: 't_pokeball', name: 'Poke Ball', category: 'TRAINER',
        onPlay: (state, playerId) => {
            // ランダムにたねポケモンカードを手札に加える (簡易的実装)
            state.effectStack.push({ type: 'DRAW_CARDS', playerId: playerId, amount: 1 });
        }
    },
    't_professor_research': {
        id: 't_professor_research', name: 'Professor\'s Research', category: 'TRAINER',
        onPlay: (state, playerId) => {
            // Pocketではトラッシュせずに2枚引く (博士の研究というよりオーキド博士)
            state.effectStack.push({ type: 'DRAW_CARDS', playerId: playerId, amount: 2 });
        }
    }
};
