...
export const SpeedRuleset: GameRuleset<SpeedState, SpeedAction> = {
  ...
  isValidAction: (state, action) => {
    ...
    if (action.type === "START") {
      return (
        ...
        Object.values(state.players || {}).filter(Boolean).length === 2
      );
    }
  ...
  reduce: (state, action, rng?: IGameRNG) => {
    ...
    if (action.type === "START") {
      ...
      players.forEach((id) => {
        ...
        newState.personalDecks[id] = createSecret(
          pDeck,
          [id],
          pDeck.map(() => "?"),
        );
        const hDeck = deck.splice(0, 5);
        newState.hands[id] = createSecret(
          hDeck,
          [id],
          hDeck.map(() => "?"),
        );
      ...
      newState.sidePiles = createSecret(
        sPiles,
        [],
        sPiles.map((p) => p.map(() => "?")) as [Card[], Card[]],
      );
    ...
    if (action.type === "PLAY") {
      ...
      if (newState.personalDecks[pId].value.length > 0) {
        ...
        newState.personalDecks[pId] = createSecret(
          pDeck,
          [pId],
          pDeck.map(() => "?"),
        );
      ...
      if (allStuck) {
        ...
        if (sPiles0.length > 0 || sPiles1.length > 0) {
          ...
          newState.sidePiles = createSecret(
            newSPiles,
            [],
            newSPiles.map((p) => p.map(() => "?")) as [Card[], Card[]],
          );
        } else {
  ...
  checkWinCondition: (state) => {
    ...
    const allStuck =
      state.playerIds.length === 2 && state.playerIds.every((id) => state.isStuck[id]);
    const sidePilesEmpty = state.sidePiles.value.every((p) => p.length === 0);
    if (allStuck && sidePilesEmpty) {
      return {
        isFinished: true,
        winnerIds: [],
        message: "It's a draw! (No more cards to flip)",
      };
    }
