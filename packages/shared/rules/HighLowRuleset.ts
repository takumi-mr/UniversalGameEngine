...
export const HighLowRuleset: GameRuleset<HighLowState, HighLowAction> = {
    ...
    isValidAction: (state, action) => {
        if (action.type === 'START') {
            return state.status === 'WAITING' && Object.values(state.players || {}).filter(Boolean).length >= 1;
        }
    ...
    reduce: (state, action, rng?: IGameRNG) => {
        if (action.type === 'START') {
            ...
            return {
                ...
                activePlayers: state.players ? (state.players[1] || state.players["1"]) ? [state.players[1] || state.players["1"]] : [] : []
            };
        }
    ...
    checkWinCondition: (state) => {
        // 先に5ポイント取ったら勝ち
        if (state.scores[1] >= 5) {
            return { isFinished: true, winnerIds: state.players?.[1] ? [state.players[1]!] : [], message: 'Player 1 Wins!' };
        }
        if (state.scores[2] >= 5) {
            return { isFinished: true, winnerIds: state.players?.[2] ? [state.players[2]!] : [], message: 'Player 2 Wins!' };
        }
        // 山札切れ
        if (state.deck.length === 0) {
            ...
            return { isFinished: true, winnerIds: winners, message: 'Deck out! Game finished.' };
        }
    ...
    getLegalActions: (state, playerId) => {
        ...
        const turn = state.currentTurn;
        const currentPlayerId = state.players ? (state.players[turn] || state.players[turn.toString() as any]) : null;
