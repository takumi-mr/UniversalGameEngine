...
describe("TicTacToeRuleset", () => {
  ...
  test("isValidAction should validate correctly", () => {
    ...
    // Valid move
    expect(
      TicTacToeRuleset.isValidAction(state, {
        type: "PLACE",
        index: 0,
        playerId: "p1",
      }),
  ...
  test("reduce should update board and turn", () => {
    ...
    const nextState = TicTacToeRuleset.reduce(state, {
      type: "PLACE",
      index: 4,
      playerId: "p1",
    });
  ...
  test("checkWinCondition should detect wins and draws", () => {
    ...
    expect(TicTacToeRuleset.checkWinCondition(state)).toEqual({
      isFinished: true,
      message: "O Wins",
      winnerIds: ["1"],
    });
