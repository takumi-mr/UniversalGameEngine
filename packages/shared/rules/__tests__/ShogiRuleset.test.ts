...
describe("ShogiRuleset", () => {
    ...
    test("isValidAction should validate basic moves", () => {
        ...
        expect(ShogiRuleset.isValidAction(state, { type: "MOVE", from: 60, to: 51, playerId: "p1" })).toBe(true);

        // Invalid: piece belongs to opponent
        expect(ShogiRuleset.isValidAction(state, { type: "MOVE", from: 20, to: 29, playerId: "p1" })).toBe(false);

        // Invalid: wrong turn
        expect(ShogiRuleset.isValidAction(state, { type: "MOVE", from: 60, to: 51, playerId: "p2" })).toBe(false);
    });
    ...
    test("DROP should work correctly", () => {
        ...
        const nextState = ShogiRuleset.reduce(state, { type: "DROP", piece: 1, to: 40 });
    ...
    test("Nifu (Double Pawn) should be invalid", () => {
        ...
        expect(ShogiRuleset.isValidAction(state, { type: "DROP", piece: 1, to: 36, playerId: "p1" })).toBe(false);
    });
});
