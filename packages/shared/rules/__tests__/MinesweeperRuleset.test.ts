...
describe("MinesweeperRuleset", () => {
  test("getInitialState should return correct initial state", () => {
    const state = MinesweeperRuleset.getInitialState({
      rows: 5,
      cols: 5,
      mineCount: 3,
    });
  ...
  test("First REVEAL should initialize mines correctly", () => {
    ...
    // Reveal (1, 1)
    const nextState = MinesweeperRuleset.reduce(state, {
      type: "REVEAL",
      row: 1,
      col: 1,
      playerId: "p1",
  ...
  test("FLAG should toggle isFlagged", () => {
    ...
    let nextState = MinesweeperRuleset.reduce(state, {
      type: "FLAG",
      row: 0,
      col: 0,
      playerId: "p1",
    ...
    nextState = MinesweeperRuleset.reduce(nextState, {
      type: "FLAG",
      row: 0,
      col: 0,
      playerId: "p1",
  ...
  test("Hit a mine should result in loss", () => {
    ...
    // Reveal a mine
    const lostState = MinesweeperRuleset.reduce(nextState, {
      type: "REVEAL",
      row: mineRow,
      col: mineCol,
      playerId: "p1",
