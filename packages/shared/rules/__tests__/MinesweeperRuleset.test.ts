...
describe("MinesweeperRuleset", () => {
    ...
    test("First REVEAL should initialize mines correctly", () => {
        const state = MinesweeperRuleset.getInitialState({ rows: 3, cols: 3, mineCount: 1 });
    ...
    test("Recursive reveal should work for 0 neighbor mines", () => {
        const state = MinesweeperRuleset.getInitialState({ rows: 3, cols: 3, mineCount: 0 });
        state.status = "PLAYING";
    ...
    test("Hit a mine should result in loss", () => {
        const state = MinesweeperRuleset.getInitialState({ rows: 3, cols: 3, mineCount: 7 }); // 7 mines in 3x3
        state.status = "PLAYING";

        // First move at (0, 0)
        const nextState = MinesweeperRuleset.reduce(state, { type: "REVEAL", row: 0, col: 0, playerId: "p1" });
    ...
    test("Clear all mines should result in win", () => {
        const state = MinesweeperRuleset.getInitialState({ rows: 3, cols: 3, mineCount: 8 });
        state.status = "PLAYING";
        state.players = { 1: "p1" };
