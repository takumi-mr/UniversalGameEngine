...
describe("TexasHoldemRuleset", () => {
    ...
    test("isValidAction should validate betting logic", () => {
        ...
        expect(TexasHoldemRuleset.isValidAction(state, { type: "RAISE", amount: 100, playerId: "p1" })).toBe(true);

        // Raise too much
        expect(TexasHoldemRuleset.isValidAction(state, { type: "RAISE", amount: 1000, playerId: "p1" })).toBe(false);
