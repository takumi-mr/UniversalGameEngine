...
- **部屋作成のレート制限・サイズ制限**: `rateLimiter.ts` の `createGameLimiter`（ユーザー ID ごと、既定 20 回/分。インスタンスローカルなインメモリ計数なので厳密ではなく乱造の抑止用）と `isCreateOptionsWithinLimit`（`options` の JSON が既定 16 KiB 以下）を Socket.io `request-create-game` / gRPC `CreateGame` の両方で通す。上限は環境変数 `CREATE_GAME_RATE_LIMIT` / `CREATE_GAME_OPTIONS_MAX_BYTES`。gRPC は RL_MODE のとき（学習ループが環境の数だけ一斉に部屋を作る）レート制限を掛けない。部屋 ID は `crypto.randomUUID()` で発行する。
- **gRPC ボット（`WaitForTurn`）へ流す局面はボット視点でマスク済み**: `botFactory.ts` の `encodeBotTurn` が `state_json` を `engine.getMaskedState(botId)` から作る（他のプレイヤーへの配信と同じ条件。観測テンソル・合法手はマスク前の状態から計算）。呼び出し元がボットのオーナーかの確認は未実装（TODO）。
- **配信される状態にサーバーシードは含まれない**: `engine.getMaskedState()` が `prngSecret` を取り除く。終局後の開示はリプレイ記録の `finalServerSeed` で行う。
- **配信メタのアクションは閲覧者ごとにマスクされる**: `broadcastLocal()` は `engine.getMaskedAction(action, viewerId)` を通してから `state-update` / `state-patch` に同梱する。ルールセットが `maskAction` を持てばそれに従う（本人にはそのまま。`null` なら同梱しない）。
- **リプレイは終局まで非公開**: `GET /replays/:gameId` は記録に `finalServerSeed`（終局時の追記でだけ付く）が無ければ 404 を返す。対局中の記録には初期状態（配札・サーバーシード）と全アクションがそのまま入っているため。
