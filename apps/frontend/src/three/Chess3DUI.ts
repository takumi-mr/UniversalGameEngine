...
export class Chess3DUI {
  ...
  private matSelected = new THREE.MeshBasicMaterial({
    color: 0xffff33,
    transparent: true,
    opacity: 0.5,
  });
  ...
  private async getOrLoadModel(val: number): Promise<THREE.Object3D | null> {
    ...
    try {
      ...
      console.log(
        `Applied glass material to piece: ${pieceName} (${color === 1 ? "White" : "Black"})`,
      );
  ...
  private createPlaceholderPiece(type: number, color: number): THREE.Object3D {
    ...
    switch (type) {
      case 1:
        geom = new THREE.CylinderGeometry(0.3, 0.35, 0.8);
        break; // Pawn
      case 2:
        geom = new THREE.BoxGeometry(0.4, 1.0, 0.6);
        break; // Knight
      case 3:
        geom = new THREE.CylinderGeometry(0.2, 0.4, 1.2);
        break; // Bishop
      case 4:
        geom = new THREE.BoxGeometry(0.5, 0.9, 0.5);
        break; // Rook
      case 5:
        geom = new THREE.CylinderGeometry(0.3, 0.4, 1.5);
        break; // Queen
      case 6:
        geom = new THREE.BoxGeometry(0.6, 1.6, 0.6);
        break; // King
      default:
  ...
  private handleMoveAttempt(from: number, to: number) {
    ...
    const isPromotionRank =
      (this.currentState.turn === 1 && Math.floor(to / 8) === 0) ||
