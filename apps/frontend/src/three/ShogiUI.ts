...
export class ShogiUI {
  ...
  private createPieceMaterial(val: number): THREE.Material {
    ...
    ctx.fillStyle =
      Math.abs(val) > 8 && Math.abs(val) !== 13 && Math.abs(val) !== 14 ? "#ff0000" : "#000000"; // 成り駒は赤（馬と龍は黒のままが多い）
    ...
    if (intersects.length > 0) {
      ...
      if (this.selectedFromIndex === null) {
        ...
        const canPromote =
          ...
          type !== 5 &&
          type !== 8 &&
          (this.isPromotionZone(fromY, this.currentState.turn) ||
            this.isPromotionZone(toY, this.currentState.turn));
