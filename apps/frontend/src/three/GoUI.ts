...
export class Go3DUI {
    ...
    private matBlack = new THREE.MeshPhongMaterial({ color: 0x111111, shininess: 80 });
    private matWhite = new THREE.MeshPhongMaterial({ color: 0xffffff, shininess: 80 });
    ...
    private initBoard() {
        ...
        for (let i = 0; i < this.size; i++) {
            ...
            // 縦線
            const vPoints = [new THREE.Vector3(pos, 0.005, -offset), new THREE.Vector3(pos, 0.005, offset)];
            const vLine = new THREE.Line(new THREE.BufferGeometry().setFromPoints(vPoints), this.matLine);
            this.scene.add(vLine);

            // 横線
            const hPoints = [new THREE.Vector3(-offset, 0.005, pos), new THREE.Vector3(offset, 0.005, pos)];
        ...
        // ホバー用のゴースト石
        this.ghostStone = new THREE.Mesh(this.stoneGeom, new THREE.MeshPhongMaterial({ transparent: true, opacity: 0.5 }));
