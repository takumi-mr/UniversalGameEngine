...
export class Equilibrium3D {
    ...
    private setupTable() {
        ...
        const ringMat = new THREE.MeshBasicMaterial({ color: 0x6366f1, side: THREE.DoubleSide, transparent: true, opacity: 0.5 });
    ...
    private createCard(name: string, x: number, y: number, z: number, colorHex: number) {
        ...
        // アニメーション用にデータを仕込む
        mesh.userData = { initialY: y, floatSpeed: Math.random() * 0.02 + 0.01, floatOffset: Math.random() * Math.PI * 2 };
