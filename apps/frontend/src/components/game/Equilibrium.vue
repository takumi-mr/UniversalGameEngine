<template>
  <div ref="containerRef" class="equilibrium-container">
    <canvas ref="canvasRef" class="webgl-canvas" />

    <div v-if="state && myData" class="hud-overlay">
      <div class="top-bar">
        <div v-for="(pData, pId) in opponentData" :key="pId" class="player-badge enemy">
          ...
          <div v-if="getRevealedCard(pData)" class="revealed-info">
      ...
      <div v-if="state.phase === 'AUCTION'" class="center-screen">
        <div v-if="isMyTurn" class="auction-panel">
          <h3>Soul Auction</h3>
          ...
          <div class="action-buttons">
            <button class="btn-primary" :disabled="myData.soulPoints < 1" @click="submitBid">
              Place Bid
            </button>
            <button class="btn-secondary" @click="passAuction">Pass</button>
          </div>
        </div>
        <div v-else class="waiting-panel">Waiting for other players to bid...</div>
      </div>
      ...
      <div class="bottom-bar">
        ...
        <div class="hand-container">
          ...
          >
            ...
            <div v-if="state.phase === 'MAIN' && isMyTurn" class="card-actions">
              ...
              <template v-else>
                ...
                <button v-if="card.type === 'GOAL'" class="btn-warning" @click="alterGoal(card.id)">
                  Set Goal
                </button>
        ...
        <div v-if="state.phase === 'MAIN' && isMyTurn" class="turn-controls">
          <button
            class="btn-sacrifice"
...
.btn-sacrifice:hover:not(:disabled) {
  background: #be123c;
}
