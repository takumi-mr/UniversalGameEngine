<template>
  <div v-if="state" class="mahjong-view">
    <!-- The Table -->
    <div class="mahjong-table-container">
      <div class="mahjong-table">
          ...
          <div class="round-indicator">
            {{
              $t("games.mahjong.round", {
                wind: $t(`games.mahjong.winds.${state.wind}`),
                round: state.round,
              })
            }}
          </div>
        ...
        <div class="player-area bottom" :class="{ 'is-active': isMyTurn }">
          ...
          <div v-if="availableActions.length > 0" class="actions-bar">
            <v-btn
          ...
          <div v-for="(score, pid) in state.scores" :key="pid" class="score-item">
            <span class="pid"
              >{{ pid
                ...
                pid === myPlayerId
                  ? ` (${$t("common.logged_in_as", { username: "" }).split(":")[0]})`
                  : ""
              }}</span
            >
...
const availableActions = computed(() => {
  ...
  if (props.state.pendingDiscard && props.state.pendingDiscard.playerId !== props.myPlayerId) {
    const hasActed = props.state.pendingDiscard.pendingActions.some(
      (a) => a.playerId === props.myPlayerId,
    );
    if (!hasActed) {
      ...
      actions.push({
        type: "CALL",
        meldType: "PON",
        tile: props.state.pendingDiscard.tile,
