<template>
  <div class="hanafuda-container">
    ...
    <div
      v-if="
        isMyTurn && (state.phase === 'CHOOSE_HAND_MATCH' || state.phase === 'CHOOSE_DECK_MATCH')
      "
      class="overlay"
    >
      <div class="modal">
    ...
    <div class="game-wrapper">
      <div class="player-area opponent-area">
        <div class="area-label">
          相手の獲得札 ({{ oppCaptured.length }}枚) / スコア:
          {{ state.yakuScores[oppId] || 0 }}
        </div>
        ...
        <div class="hand-area">
          <div v-for="(_, i) in oppHand" :key="`opp-hand-${i}`" class="card hidden" />
        </div>
      </div>
      ...
      <div class="player-area my-area">
        ...
        <div class="area-label">
          自分の獲得札 ({{ myCaptured.length }}枚) / スコア:
          {{ state.yakuScores[myId] || 0 }}
        </div>
      </div>
    </div>
  </div>
