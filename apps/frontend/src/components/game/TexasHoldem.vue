...
const canCall = computed(() => callAmount.value > 0 && props.state.playerChips[myPlayerId.value] >= callAmount.value);
const canRaise = computed(() => props.state.playerChips[myPlayerId.value] > callAmount.value);

// レイズ額の管理
const minRaise = computed(() => Math.min(props.state.playerChips[myPlayerId.value], callAmount.value + 10)); // 最低レイズ額の簡易計算
...
const takeAction = (type: TexasHoldemAction['type']) => {
  ...
  if (type === 'RAISE') {
    emit('action', { type, amount: raiseAmount.value, playerId: myPlayerId.value });
  } else {
...
const formatPhase = (phase: string) => {
  const map: Record<string, string> = {
    'PRE_FLOP': 'プリフロップ', 'FLOP': 'フロップ', 'TURN': 'ターン', 'RIVER': 'リバー', 'SHOWDOWN': 'ショーダウン'
  };
...
.pot-label { font-size: 0.7rem; color: #94a3b8; letter-spacing: 1px; }
.pot-amount { font-size: 1.5rem; font-weight: bold; color: #fbbf24; }
...
.player-seat.is-active { border-color: #3b82f6; box-shadow: 0 0 20px rgba(59, 130, 246, 0.4); }
.player-seat.is-folded { opacity: 0.5; filter: grayscale(1); }
...
.player-chips { color: #fbbf24; font-size: 0.8rem; }

.hole-cards { display: flex; gap: 4px; }
.small-card { width: 40px; height: 56px; }
...
.my-player-area.is-my-turn { box-shadow: inset 0 4px 20px rgba(59, 130, 246, 0.3); }
.my-player-area.is-folded { opacity: 0.6; }
...
.my-stats { display: flex; flex-direction: column; gap: 4px; }
.my-name { font-size: 1.2rem; font-weight: bold; color: #e2e8f0; }
.my-chips { color: #fbbf24; }
.my-bet { color: #94a3b8; font-size: 0.9rem; }

.my-cards .playing-card { width: 80px; height: 112px; transform: translateY(-20px); box-shadow: 0 10px 20px rgba(0,0,0,0.5); }
...
.btn {
  padding: 12px 24px; border-radius: 8px; font-weight: bold; border: none; cursor: pointer;
  font-size: 1rem; color: white; transition: all 0.2s;
}
.btn:hover { transform: translateY(-2px); }
.btn-fold { background: #475569; }
.btn-fold:hover { background: #64748b; }
.btn-check { background: #10b981; }
.btn-check:hover { background: #34d399; }
.btn-call { background: #3b82f6; }
.btn-call:hover { background: #60a5fa; }
.btn-raise { background: #8b5cf6; }
.btn-raise:hover { background: #a78bfa; }

.raise-container { display: flex; flex-direction: column; gap: 8px; align-items: center; }
.raise-slider { width: 150px; cursor: pointer; }

/* --- Card Inner CSS --- */
:deep(.card-inner) {
  width: 100%; height: 100%; background: white; border-radius: inherit;
  position: relative; padding: 4px; box-sizing: border-box; color: #1a1a1a;
}
:deep(.card-inner.is-red) { color: #e11d48; }
:deep(.card-inner.is-back) { background: #1e3a8a; border: 4px solid white; }
...
:deep(.top-left) { position: absolute; top: 4px; left: 6px; text-align: center; line-height: 1; }
:deep(.rank) { font-size: 1.1rem; font-weight: 900; }
:deep(.suit) { font-size: 0.9rem; }
:deep(.center-suit) { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); font-size: 2rem; opacity: 0.8; }
</style>