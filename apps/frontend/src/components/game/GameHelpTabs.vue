<template>
  <div class="game-help-tabs">
    <v-tabs v-if="hasTabs" v-model="activeTab" color="primary" grow class="mb-4">
      <v-tab v-for="(_, key) in tabs" :key="key" :value="key">
    ...
    <div v-if="hasTabs">
      <v-window v-model="activeTab">
        <v-window-item v-for="(_, key) in tabs" :key="key" :value="key">
...
const getTabIcon = (key: string | number) => {
  ...
  if (k.includes("win") || k.includes("goal"))
    return { icon: "mdi-trophy-outline", color: "warning" };
  ...
  if (k.includes("special") || k.includes("phase"))
    return { icon: "mdi-star-outline", color: "success" };
