...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  connect: (gameId: string, options?: { asSpectator?: boolean }) =>
    ipcRenderer.invoke("grpc:connect", gameId, options),
  disconnect: () => ipcRenderer.invoke("grpc:disconnect"),
  sendAction: (action: unknown) => ipcRenderer.invoke("grpc:sendAction", action),
  sendChat: (message: string, channel: string, recipientId?: string) =>
    ipcRenderer.invoke("grpc:sendChat", message, channel, recipientId),
