...
## 🖼️ レンダラー非依存 (Renderer-Agnostic)

本プロジェクトのフロントエンドは、特定のレンダリングライブラリに依存しない設計を採用しています。ゲームごとに最適な描画手法を選択したり、複数のライブラリを共存させたりすることが可能です。

- **柔軟なエンジン切り替え**: Three.js から Unity (WebGL)、Babylon.js、PixiJS への移行や追加が、コアロジックに影響を与えずに行えます。
- **ライブラリの共存**: 
    - 3Dボードゲーム: **Three.js** (`Go.vue`, `Othello3D.vue`)
    - 2Dカードゲーム: **DOM/CSS** (`Uno.vue`, `TicTacToe.vue`)
    - 高度なグラフィックス: **Unity** (将来的な拡張)
- **カプセル化された描画ロジック**: 各ゲームの描画ロジックはコンポーネントまたは専用のUIクラス（例: `GoUI.ts`）に封じ込められており、他のゲームのレンダリングに干渉しません。
